"""Surveillance proactive des jobs ETL (#157, ADR-0022).

Le bot reste pur client HTTP : c'est lui qui interroge périodiquement
`/etl/jobs` et pousse une alerte dans le chat de notification
...
async def verifier_jobs(bot, client: ElectriCoreClient, vus: set[str], chat_id: str) -> None:
    ...
    for job in jobs:
        ...
        if job["status"] != "failed":
            continue
        texte = f"🚨 Job ETL <code>{escape(job['mode'])}</code> en échec — <code>{job['id'][:8]}</code>"
        if job.get("error"):
            texte += f"\n\n<code>{escape(job['error'][:500])}</code>"
        try:
            await bot.send_message(chat_id=chat_id, text=texte, parse_mode="HTML")
        except Exception:
            logger.exception("Échec d'envoi de l'alerte ETL")
...
async def boucle_surveillance(bot, chat_id: str) -> None:
    ...
    logger.info("Surveillance ETL active (chat %s)", chat_id)
    while True:
