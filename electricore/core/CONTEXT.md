...
**Taux en vigueur** :
Valeur d'un taux réglementé (Accise, CTA, TURPE…) applicable à une date donnée. Les taux changent par arrêté ou décret CRE : les fichiers `*_rules.csv` versionnent ces changements en stockant la date d'**entrée en vigueur** (`start`). Pour l'Accise et la CTA, chaque ligne remplace la précédente jusqu'à la ligne suivante (ou indéfiniment pour la dernière) — pas de colonne `end`, les taux régulés sont continus dans le temps ; `turpe_rules.csv` porte en revanche des fenêtres `start`/`end` par grille tarifaire. `core/pipelines/taux.py` est le **registre** des taux régulés : `charger_regles_taux(nom_fichier, taux_col)` lit un `*_rules.csv` (Accise/CTA) en historique typé `(start, <taux>, reference)`, et `ajouter_taux_en_vigueur()` y sélectionne « le taux en vigueur à la date X » (`join_asof` backward). TURPE garde son propre lecteur (`load_turpe_rules`, grille 2-D FTA × `start`/`end`).
...
**Mémo de puissance** (`memo_puissance`) :
Résumé humain, dans l'onglet *Changements puissance*, de la **répartition mensuelle** de la puissance souscrite quand elle change en cours de mois — les sous-périodes d'abonnement, **date de bascule intercalée** : « 3 jours à 6kVA, puis le 08/07/2026 : 27 jours à 9kVA ». La date = `date_evenement` de la *Rupture d'abonnement* (`MCT` puissance) — à distinguer de la *répartition* elle-même. Vide s'il n'y a pas de vrai changement de puissance (un `MCT` de FTA seule à puissance égale n'en produit pas — gate `n_unique(puissance) > 1`).
...
Bundle immutable des éléments dérivés une seule fois pour produire la facturation d'un mois donné : `historique_enrichi`, `abonnements`, `energie`, `releves_utilises` (journal des relevés effectivement consommés par la chronologie — traçabilité des index, ADR-0029) et `facturation_mensuelle` (méta-périodes du mois) — **cinq** frames. Une composition pure dans `core/builds/contexte_mensuel.py` (cf. [ADR-0019](../../docs/adr/0019-roles-loaders-pipelines-builds-integrations.md)) — `charger(historique, releves, mois)`, le **seam profond** qui porte tout l'assemblage, frames fournis par l'appelant (tests, notebooks, source alternative) — et **deux adaptateurs d'I/O fins** par-dessus, qui résolvent les sources par défaut (loaders DuckDB) puis délèguent à `charger()` (issue #145, scindage pur/I-O prévu par les « Limites » d'ADR-0019) : `contexte_du_mois(mois)` pour le **parc entier** et `contexte_du_mois_filtre(pdl, rsc)` pour un **point/contrat unique** (drill-down `/chronologie`, #366, prédicat poussé dans DuckDB). Les deux entrées I/O restent **distinctes par choix** ([ADR-0050](../../docs/adr/0050-entrees-contexte-mensuel-parc-et-point-distinctes.md)) — appelants et paramètres disjoints, faces publiques de deux flux, sur un boundary d'I/O volontairement fin (pas de la duplication à effondrer). Partagé par les builds qui touchent au même mois (rapprochement, documents de campagne, taxes mensuelles) pour ne déclencher le pipeline `facturation()` qu'une seule fois.
