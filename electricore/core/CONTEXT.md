...
**Voie communicante** vs **Voie non-communicante** (routage de l'énergie, pas de l'abonnement) :
...
**Pas de gate au grain relevé** : le pipeline énergie calcule *toutes* les sous-périodes ; le relevé porté par le C15 à l'activation du calendrier Distributeur (`CMAT`, déjà `impacte_energie` par changement de calendrier) borne proprement la première sous-période communicante. Une période qui enjamberait l'ère non-communicante retombe, par son *mois de début*, dans un mois non-communicant déjà écarté du réel — donc inoffensive. La sélection vit **au grain méta**, pas en filtrage fragile au grain relevé.
