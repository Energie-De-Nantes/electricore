...
Ligne de temps **énergie** d'un contrat : tous les relevés d'index aux dates qui bornent ses périodes, lus depuis le **modèle de relevés canonique** dbt `releves` (loader `releves()`, [ADR-0029](../../docs/adr/0029-modele-releves-canonique-dbt-assemble-coeur-arbitre.md)) — qui réunit déjà relevés C15 *avant*/*après* et relevés périodiques (R151/R64) —, assemblés, dédoublonnés par source prioritaire et ordonnés. L'assemblage des sources et l'harmonisation des dates sont désormais portés **par dbt** (fin de l'extraction C15 depuis l'historique + interrogation R151/R64, retirée en #248) ; le cœur **arbitre** sur ce modèle. Entrée pure de `calculer_periodes_energie`, produite par `chronologie_releves()` (`core/pipelines/energie.py`), validée par le schéma `ChronologieReleves` qui matérialise comme **contrat** cinq invariants jadis encodés incidemment : grain d'**1 ligne par `(ref_situation_contractuelle, date_releve, ordre_index)`**, RSC **non-null** (attribution garantie en amont — valeur **native** des relevés C15 + requête FACTURATION qui porte RSC/FTA/niveau du contrat depuis le substrat d'événements ; le mart `releves` ne recopie plus la situation, ADR-0029/[ADR-0039](../../docs/adr/0039-chronologie-substrat-attributs-situation-hors-mart.md) ; la chronologie ne ré-attribue pas), `source` dans une **énumération fermée** choisie par la **table de priorité explicite** `flux_C15 > flux_R64 > flux_R151` (`PRIORITE_SOURCES`, ADR-0028 — fin du tri alphabétique qui faisait gagner R151 sur R64 par accident lexical), tolérance d'appariement asof **±4 h nommée** (`TOLERANCE_APPARIEMENT_RELEVES`, absorbe le décalage C15 00:01 / R151 02:00), et `ordre_index` **booléen** (`False` = avant / relevé périodique, `True` = après C15 — discriminant unifié de bout en bout). Le split par source, la résolution asof et le dédoublonnage sont de l'*implémentation* derrière cette interface (`_assembler_chronologie`).
...
Ligne de temps **complète** d'une *situation contractuelle* (RSC) : **tous** ses faits ordonnés — événements C15 (y compris hors-comptage : `MDPRM` de niveau, `MCT` puissance seule), relevés d'index, et attributs de situation (FTA, *niveau d'ouverture*…) propagés depuis la **source unique** des événements (et non re-dérivés ailleurs). Bornée entrée→sortie de la RSC. C'est le **substrat** dont on *dérive* les périodisations (abonnement, énergie, NC), chacune avec son propre découpage — partager le substrat n'est **pas** unifier les découpages, qui restent séparés ([ADR-0023](../../docs/adr/0023-periodisations-separees-abonnement-energie.md)). Les attributs de situation appartiennent à ce substrat, **pas** au relevé ([ADR-0039](../../docs/adr/0039-chronologie-substrat-attributs-situation-hors-mart.md) — fin de la recopie sur le mart `releves`). La *Chronologie des relevés* en est la **projection énergie** (restreinte aux relevés + événements impactant le comptage).
_Éviter_ : timeline (anglicisme) ; confondre le *substrat* (cette chronologie) avec une *périodisation* (un découpage qui en dérive).
...
Borne temporelle unique (`datetime` en Europe/Paris) qui paramètre `pipeline_historique` : elle borne *à la fois* les événements C15 retenus (`date_evenement <= horizon`) *et* la fin par défaut des périodes ouvertes — les PDL non résiliés sont facturés jusqu'au 1ᵉʳ du mois de l'horizon (génération des événements FACTURATION). À horizon fixé, le pipeline est une transformation **pure** : deux exécutions donnent une sortie identique, indépendamment de l'heure d'exécution (ADR-0019). Le défaut « 1ᵉʳ du mois courant » est résolu en **vraie heure de Paris** (`horizon_par_defaut()`) une seule fois, au boundary I/O — `charger()` / `contexte_du_mois()` dans `core/builds/contexte_mensuel.py` — puis propagé. Unifie et remplace l'ancien `date_limite` (qui ne bornait que les événements) et le défaut de fin de période lu séparément à l'horloge : leurs deux lectures divergeaient de fuseau (heure murale UTC renommée Paris → décalage 1–2 h, incohérence possible autour de minuit le 1ᵉʳ du mois). Corrigé par l'issue #179.
_Éviter_ : `date_limite` (ancien nom, ne couvrait que le filtrage des événements), « date du jour » / `now()` dans un pipeline (rompt la pureté).
