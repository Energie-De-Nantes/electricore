...
**Relais aveugle** :
Extension du *flux aveugle* (ADR-0037 §Extension) au relais, elle-même étendue au déchiffrement ([#692](https://github.com/Energie-De-Nantes/electricore/issues/692)) : un run qui n'a **rien livré** (0 push réussi) malgré **au moins un échec**, de push OU de déchiffrement — `StatsRelais.relais_aveugle()` = `pousses == 0 ET (echecs_push + len(zips_indechiffrables)) > 0`. La part déchiffrement se lit sur les **zips indéchiffrables** (les noms qui alimentent le journal `statut='echec'`), jamais sur un compteur : une seule vérité pour l'escalade, le journal et le résumé CLI ([#695](https://github.com/Energie-De-Nantes/electricore/issues/695) — le pourquoi de cette unification vit dans les docstrings de `StatsRelais`). L'escalade **s'arrête au processus** : `main()` (`__main__.py`) sort en code non-zéro, systemd marque l'unité `failed`. Avant #692, une rotation de clé Enedis (tous les zips indéchiffrables, aucun n'atteignant jamais l'étage push) affichait `✅ Relais : 0 candidat(s)…` et sortait 0 — **pour toujours** : le scénario fondateur de l'escalade, manqué côté déchiffrement, le reproche exact adressé à inotify dans #637. Un échec **isolé** noyé dans des push réussis (`pousses > 0`) n'est **pas** aveugle — toléré, retenté au run suivant par le balayage réconciliant ; **même règle** pour un échec de déchiffrement isolé (sémantique tolérante inchangée par #692). Pas de surface API/bot pour cette escalade : le relais est délibérément hors de la stack d'ingestion, et la chaîner à la surveillance bot demanderait un journal interrogeable côté bot (tranche distincte, non retenue).
_Éviter_ : croire que l'escalade remonte au bot (elle s'arrête à `journalctl`/`systemctl status`) ; confondre avec le *flux aveugle* de l'ingestion (même forme de prédicat, deux compteurs et deux processus différents) ; croire que seul le push est compté dans le prédicat (le déchiffrement l'est aussi depuis #692), ou qu'un compteur de la brique decrypt gouverne la décision (ce sont les noms journalisés, #695).
