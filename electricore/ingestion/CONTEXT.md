...
**Relais de flux** :
Outil autonome, pose-et-oublie, qui recopie les flux Enedis **déjà déchiffrés** vers le SFTP d'un partenaire — conteneurisé depuis [#657](https://github.com/Energie-De-Nantes/electricore/issues/657) : timer systemd dédié → `docker compose run --rm` (mini-compose `deploy/relais/compose-relais.yml`, tag `RELAIS_VERSION` découplé d'`ELECTRICORE_VERSION`), pipeline dlt et DuckDB **distincts** de l'ingestion principale ([`electricore/ingestion/relais/pipeline.py`](relais/pipeline.py)). Réutilise les briques stables de l'ingestion (`crypto`, `archive`, `create_sftp_resource`, `etape_chaine`) mais n'importe **jamais** son runner, son curseur ni son état — garde d'indépendance vérifiée statiquement et dynamiquement (`tests/ingestion/test_relais_independance.py`, `MODULES_INTERDITS`) : une évolution du runner ne doit jamais casser le relais sans qu'on l'ait touché. Boucle de **balayage réconciliant** (re-liste toute la source à chaque run, `incremental=False`), pas d'inotify — la continuité tient par le re-listing, pas par la détection d'événements ([#637](https://github.com/Energie-De-Nantes/electricore/issues/637)). Un ELT à lui seul depuis [#646](https://github.com/Energie-De-Nantes/electricore/issues/646) (mouvement → journal → *vue d'audit du relais*), deux orchestrations de briques communes avec l'ingestion.
_Éviter_ : le confondre avec l'ingestion elle-même (DuckDB, `flux_raw`, dbt des flux linéarisés) — le relais ne produit qu'un journal de livraisons et sa vue d'audit passive, aucune table exploitable par les pipelines core.

**Journal enrichi** :
Tout zip **vu** au balayage entre dans le journal (`relais_livraisons`), pas seulement les livrés — `statut` ∈ {`vu`, `pousse`, `amorce`, `echec`} ([#646](https://github.com/Energie-De-Nantes/electricore/issues/646)). `vu` : zip listé mais jamais candidat au push (exclu par le filtre `flux`, ou flux non encore couvert côté ingestion comme R17) — journalisé une seule fois, pas rejoué à chaque passage. `echec` couvre **deux étages** depuis [#692](https://github.com/Energie-De-Nantes/electricore/issues/692) : un zip **indéchiffrable** (trousseau incomplet, rotation de clé Enedis — le décryptage ne produit rien ; journalisé par un wrapper observateur autour de la brique decrypt, la sortie du decrypt alimentant directement le push, un échec n'y transite donc pas naturellement) ou un push tenté et raté (cible injoignable, contrôle intra-zip, vérification d'écriture). Dans les deux cas retenté au passage suivant par le balayage réconciliant, jamais silencieusement perdu — une ligne par zip et par run, **sans dédup** (le journal est append-only, la répétition d'un `echec` trace la durée d'un trou de clé ou d'une panne de cible). Motivation : l'audit de réception (*vue d'audit du relais*) porte sur ce qu'Enedis a **émis**, pas seulement sur ce que le relais a réussi à pousser — un zip en échec prouve quand même l'existence de son numéro de séquence. La vérification de complétude (`zips_non_relayes()`) sait restreindre l'écart aux flux **dus** au partenaire (`flux_filtres`, même sémantique que le filtre `flux`, [#683](https://github.com/Energie-De-Nantes/electricore/issues/683)) — sans ce filtre, les zips `vu` hors liste (X13, LTE01, R63…) noient les vrais manquants.
_Éviter_ : filtrer `zips_non_relayes()` sur *toute* présence en journal (un zip `vu`/`echec` n'est **pas** relayé, seuls `pousse`/`amorce` comptent) — piège symétrique à celui déjà documenté pour l'amorçage ; croire qu'un zip indéchiffrable peut atterrir `'vu'` — l'échec de déchiffrement est journalisé **avant** la passe `'vu'` du même run, jamais l'inverse ([#692](https://github.com/Energie-De-Nantes/electricore/issues/692)).

**Vue d'audit du relais** :
`journal.relais_audit_sequences`, matérialisée par un mini-projet dbt **embarqué** (`electricore/ingestion/relais/dbt/`) en fin de chaque passage de balayage, même process — connexion dbt ouverte puis refermée par la passe, la base journal reste requêtable entre deux passages du timer ([#646](https://github.com/Energie-De-Nantes/electricore/issues/646)). Réutilise la macro `audit_sequences` du projet dbt principal ([#645](https://github.com/Energie-De-Nantes/electricore/issues/645)) en **package local** (`packages.yml` → `local:`) — zéro règle de nomenclature dupliquée. Le journal n'a pas de table par flux (contrairement à l'ingestion) : le code flux est dérivé du nom de zip lui-même. Vue **passive**, comme côté ingestion : aucune alerte de trous côté relais, sa supervision reste `StatsRelais`/le code de sortie du processus — la vue sert la consultation à la demande (rapprochement Haulogy). Dégradée (loggée, jamais levée) sur échec dbt : la mission de mouvement du relais ne dépend jamais de cet étage.
_Éviter_ : câbler un data test `aucune_anomalie` sur cette vue (romprait la propriété passive) ; croire qu'un échec dbt fait échouer le relais (seul `relais_aveugle()` gouverne l'escalade).

**Amorçage** :
Acte **unique** (`relais seed --avant <date>`) qui marque tous les zips de la source antérieurs à une date comme livrés **sans les pousser** — évite de noyer le partenaire dans tout l'historique au tout premier run. Remplace un ancien filtre `depuis` en config (retiré, [#643](https://github.com/Energie-De-Nantes/electricore/issues/643)) : un knob de config **permanent** était la mauvaise forme pour un geste qui ne doit se produire **qu'une fois**, à la mise en service. Écrit dans le **même état** `zips_livrés` que le push normal (sinon le run périodique repousserait tout ce que le seed vient de marquer) et journalise avec `statut='amorce'` (distinct de `statut='pousse'`, un push réel) — sans cette ligne de journal, `zips_non_relayes()` (vérification de complétude) remonterait à vie des milliers de faux positifs sur les zips amorcés. La sortie CLI du seed annonce son compte (« ✅ Amorçage : N zip(s) marqués livrés », [#684](https://github.com/Energie-De-Nantes/electricore/issues/684)) — le chiffre de contrôle immédiat, sans requête duckdb manuelle. **Refuse par défaut** si le journal contient déjà des livraisons (`--force` outrepasse) : c'est la seule opération du relais capable de fabriquer un « oubli définitif » — lancée par erreur après la mise en service, elle enterrerait silencieusement tout ce qui restait à relayer. `pipelines_dir` est épinglé à `destination_db.parent` (pas le défaut `~/.dlt/pipelines`, dépendant du `HOME` de l'utilisateur qui lance) : c'est ce qui garantit que le seed (`docker compose run` manuel) et le timer (même conteneur, uid 1000) lisent et écrivent le **même** état — `/data` du volume nommé `relais_data` depuis [#657](https://github.com/Energie-De-Nantes/electricore/issues/657).
...
Après chaque dépôt SFTP côté relais, la taille distante est comparée à la taille locale juste écrite ([#646](https://github.com/Energie-De-Nantes/electricore/issues/646)) — un dépôt tronqué **n'est pas** marqué livré, retenté au passage suivant par le balayage réconciliant. Même direction d'échec sûre qu'une cible injoignable : jamais un oubli, toujours un re-push.

**Contrôle intra-zip (relais)** :
Au dézippage, avant tout push, le relais vérifie que le *compteur intra-zip* (`_XXXXX_YYYYY`, glossaire « Complétude des flux ») est complet — **R151 excepté** (compteur inter-zips, sans objet à l'intérieur d'un seul zip). F15 : vérifie en plus la présence d'un fichier de données générales, distinct des fichiers de détail numérotés. Toute incohérence → **rien n'est poussé** pour ce zip, échec compté (`StatsRelais.echecs_push`) et journalisé (`statut='echec'`), alerté (log `WARNING`, même canal que tout échec de push) — jamais un lot partiel silencieux chez le partenaire.
_Éviter_ : confondre avec le contrôle intra-zip *rétrospectif* du mart d'ingestion (`audit_sequences`, #645) — celui-ci constate après coup depuis le landing ; celui du relais bloque *avant* le push, en amont de toute persistance.

**Relais aveugle** :
Extension du *flux aveugle* (ADR-0037 §Extension) au relais, elle-même étendue au déchiffrement ([#692](https://github.com/Energie-De-Nantes/electricore/issues/692)) : un run qui n'a **rien livré** (0 push réussi) malgré **au moins un échec**, de push OU de déchiffrement — `StatsRelais.relais_aveugle()` = `pousses == 0 ET (echecs_push + len(zips_indechiffrables)) > 0`. Vérité unique ([#695](https://github.com/Energie-De-Nantes/electricore/issues/695)) : `zips_indechiffrables` — les **noms** collectés par le wrapper observateur `_dechiffrer_et_observer` — est la SEULE source du déchiffrement pour cette décision et pour le résumé CLI ; `StatsRelais.chaine` (composition — un `StatsChaine` interne passé à la brique decrypt de `crypto.py`, réutilisée telle quelle par ce même wrapper, aucune brique partagée modifiée) reste **purement interne** — `chaine.echecs_dechiffrement` n'est affiché nulle part et aucune décision n'en dépend ; `chaine.fichiers` (zips entrés au déchiffrement) en découle gratuitement, affiché dans le résumé CLI. L'escalade **s'arrête au processus** : `main()` (`__main__.py`) sort en code non-zéro, systemd marque l'unité `failed`. Avant #692, une rotation de clé Enedis (tous les zips indéchiffrables, aucun n'atteignant jamais l'étage push) affichait `✅ Relais : 0 candidat(s)…` et sortait 0 — **pour toujours** : le scénario fondateur de l'escalade, manqué côté déchiffrement, le reproche exact adressé à inotify dans #637. Un échec **isolé** noyé dans des push réussis (`pousses > 0`) n'est **pas** aveugle — toléré, compté, retenté au run suivant par le balayage réconciliant ; **même règle** pour un échec de déchiffrement isolé (sémantique tolérante inchangée par #692). Pas de surface API/bot pour cette escalade : le relais est délibérément hors de la stack d'ingestion, et la chaîner à la surveillance bot demanderait un journal interrogeable côté bot (tranche distincte, non retenue).
_Éviter_ : croire que l'escalade remonte au bot (elle s'arrête à `journalctl`/`systemctl status`) ; confondre avec le *flux aveugle* de l'ingestion (même forme de prédicat, deux compteurs et deux processus différents) ; croire que seul le push est compté dans le prédicat (le déchiffrement l'est aussi depuis #692) ; croire que `chaine.echecs_dechiffrement` gouverne quoi que ce soit — depuis #695 c'est `len(zips_indechiffrables)`, le compteur de la brique est un log interne.
