-- Chronologie des relevés — MART MINCE (ADR-0041, normalisé ADR-0045 #431) : projection
-- ÉNERGIE de la spine, assemblée entièrement en dbt. Une ligne par relevé qui borne une
-- période d'énergie :
...
--
-- NORMALISATION (ADR-0045) : ce mart ne porte QUE l'identité + la **situation** (FTA, niveau,
-- RSC — état natif C15 ou ffill-spine) + **`releve_id` (référence)** + les attributs de slot
-- (`ordre_index`, `source`, `releve_manquant`). Le payload d'index pur (`index_*_kwh`,
-- `nature_index`, `id_calendrier_distributeur` — fonction 1:1 de `releve_id`) N'est PLUS
-- recopié ici : la vue `chronologie_releves` le ré-attache par star-join sur `releves`
-- (source de vérité unique, ADR-0029/0038). Le cœur lit la vue, pas ce mart.
--
