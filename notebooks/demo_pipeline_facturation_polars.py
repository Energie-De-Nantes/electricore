...
def _():
    ...
    from electricore.core.pipelines_polars.abonnements_polars import pipeline_abonnements as pipeline_abonnements_polars
...
def load_data(
    ...
    df_historique_polars = _lf_historique.collect()

    # Chargement relevés
    _lf_releves = r151().lazy()
    df_releves_polars = _lf_releves.collect()

    print(f"✅ Historique: {len(df_historique_polars)} lignes")
    print(f"✅ Relevés: {len(df_releves_polars)} lignes")

    # Conversion pour pandas avec mapping colonnes
    df_historique_pandas = df_historique_polars.to_pandas()
    df_releves_pandas = df_releves_polars.to_pandas()
...
@app.cell
def _(df_historique_polars, df_releves_polars):
    print("🔍 Aperçu des données Polars:")
    print(f"Historique: {df_historique_polars.shape}")
    print(df_historique_polars.head(3))
    print(f"\nRelevés: {df_releves_polars.shape}")
    print(df_releves_polars.head(3))
    return
...
def execute_polars_pipeline(
    ...
    _lf_historique = df_historique_polars.lazy()
    _lf_releves = df_releves_polars.lazy()

    # Étape 1: Calculer les périodes d'abonnement
    _lf_abonnements = pipeline_abonnements_polars(_lf_historique)
    df_abonnements_polars = _lf_abonnements.collect()

    # Étape 2: Calculer les périodes d'énergie
    _lf_energies = pipeline_energie_polars(_lf_historique, _lf_releves)
    df_energies_polars = _lf_energies.collect()

    # Étape 3: Pipeline facturation
    df_facturation_polars = pipeline_facturation_polars(_lf_abonnements, _lf_energies)

    _end_time = time.time()
    time_polars = _end_time - _start_time

    print(f"✅ Pipeline Polars terminé en {time_polars:.3f}s")
    print(f"   - Abonnements: {len(df_abonnements_polars)} périodes")
    print(f"   - Énergies: {len(df_energies_polars)} périodes")
    print(f"   - Facturation: {len(df_facturation_polars)} méta-périodes")
    return df_facturation_polars, time_polars


@app.cell
def _(time_pandas, time_polars):
    print(f"⏱️ Comparaison des performances:")
    print(f"   - Pandas: {time_pandas:.3f}s")
    print(f"   - Polars: {time_polars:.3f}s")
    if time_polars > 0:
        _speedup = time_pandas / time_polars
        print(f"   - Speedup: {_speedup:.2f}x")
    return


@app.cell
def _(df_facturation_pandas, df_facturation_polars):
    print("🔍 Aperçu des résultats facturation:")
    print("\n📊 Pandas (5 premières lignes):")
    print(df_facturation_pandas.head())
    print(f"\n⚡ Polars (5 premières lignes):")
    print(df_facturation_polars.head())
    return


@app.cell
def compare_results(df_facturation_pandas, df_facturation_polars, pd, pl):
    ...
    _polars_totals = {
        'turpe_fixe': df_facturation_polars['turpe_fixe'].sum(),
        'turpe_variable': df_facturation_polars['turpe_variable'].sum(),
        'base_energie': df_facturation_polars.get('base_energie', pl.Series([0])).sum(),
        'nb_jours': df_facturation_polars['nb_jours'].sum(),
        'nb_meta_periodes': len(df_facturation_polars)
    }

    print("Pandas vs Polars:")
    for key in _pandas_totals:
        _p_val = _pandas_totals[key]
        _pl_val = _polars_totals[key]
...
@app.cell
def _(df_facturation_pandas, df_facturation_polars, go, pd):
    ...
    _df_plot_polars = df_facturation_polars.to_pandas() if hasattr(df_facturation_polars, 'to_pandas') else df_facturation_polars
    _df_plot_polars['pipeline'] = 'Polars'
    _df_plot_polars['debut'] = pd.to_datetime(_df_plot_polars['debut'])
    ...
    _df_sample_polars = _df_plot_polars.head(_sample_size)
    ...
    # Timeline polars
    fig_timeline.add_trace(go.Scatter(
        x=_df_sample_polars['debut'],
        y=_df_sample_polars['puissance_moyenne'],
        mode='markers+lines',
        name='Polars',
...
@app.cell
def _(df_facturation_pandas, df_facturation_polars, pd, px):
    ...
    _df_dist_polars = pd.DataFrame({
        'puissance_moyenne': df_facturation_polars['puissance_moyenne'].to_list() if hasattr(df_facturation_polars['puissance_moyenne'], 'to_list') else df_facturation_polars['puissance_moyenne'],
        'pipeline': 'Polars'
    })

    _df_combined = pd.concat([_df_dist_pandas, _df_dist_polars], ignore_index=True)
...
@app.cell
def _(go, time_pandas, time_polars):
    """Graphique de performance"""

    fig_perf = go.Figure(data=[
        go.Bar(
            x=['Pandas', 'Polars'],
            y=[time_pandas, time_polars],
            marker_color=['blue', 'red'],
            text=[f'{time_pandas:.3f}s', f'{time_polars:.3f}s'],
            textposition='auto'
        )
    ])
