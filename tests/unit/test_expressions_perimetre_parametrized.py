...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([6.0, 6.0, 6.0, 6.0], [False, False, False, False], "sequence_constante"),
        # Cas changements multiples successifs
        ([3.0, 6.0, 9.0, 12.0], [False, True, True, True], "changements_successifs"),
        # Cas valeur unique - pas de changement
        ([9.0], [False], "valeur_unique"),
    ],
...
def test_expr_changement_cases(valeurs, expected_changements, description):
    ...
    assert result["changement"].to_list() == expected_changements, f"Échec pour le cas: {description}"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["BTINFCU4", "BTINFCU4", "BTINFMU4"], "FTA", ["", "", "FTA: BTINFCU4 → BTINFMU4"]),
        # Cas avec nombres (puissance)
        ([6.0, 9.0, 12.0], "P", ["", "P: 6.0 → 9.0", "P: 9.0 → 12.0"]),
        # Cas sans changement
        (["MÊME", "MÊME", "MÊME"], "TEST", ["", "", ""]),
        # Cas avec nulls
        ([None, "VAL", None], "X", ["", "", ""]),
...
def test_expr_resume_changement_formats(valeurs, label, expected_resumes):
    ...
    result = df.select(expr_resume_changement("colonne", label).alias("resume"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["MES", "MCT", "RES", "AUTRE"], [True, False, True, False]),
        # Cas avec PMES/CFNE/CFNS
        (["PMES", "CFNE", "CFNS", "MCT"], [True, True, True, False]),
        # Cas sans événements structurants
        (["MCT", "MCT", "MCT"], [False, False, False]),
        # Cas nulls (retourne null, pas False)
        ([None, "MES", None], [None, True, None]),
    ],
)
def test_expr_evenement_structurant_detection(evenements, expected_structurants):
    ...
    result = df.with_columns(expr_evenement_structurant().alias("structurant"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["MES", "MCT", "RES"], [False, False, False], [False, False, False], [True, False, True]),
        # Cas changement puissance uniquement
        (["MCT", "MCT", "MCT"], [False, True, False], [False, False, False], [False, True, False]),
        # Cas changement FTA uniquement
        (["MCT", "MCT", "MCT"], [False, False, False], [False, True, False], [False, True, False]),
        # Cas combinés
        (["MES", "MCT", "MCT"], [False, True, False], [False, False, True], [True, True, True]),
    ],
)
def test_expr_impacte_abonnement_logique(evenements, changements_puissance, changements_fta, expected_impacte):
...
@pytest.mark.unit
@pytest.mark.parametrize(
    "taille_df", [0, 1, 2, 100, 1000], ids=["vide", "une_ligne", "deux_lignes", "cent_lignes", "mille_lignes"]
)
...
@pytest.mark.unit
@pytest.mark.parametrize("nb_groupes", [1, 2, 5, 10], ids=["un_groupe", "deux_groupes", "cinq_groupes", "dix_groupes"])
