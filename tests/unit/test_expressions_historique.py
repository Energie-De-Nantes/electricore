...
def test_expr_changement_avant_apres_detecte_changements():
    ...
    result = df.select(expr_changement_avant_apres("avant_Calendrier", "apres_Calendrier").alias("changement"))
...
def test_expr_changement_avant_apres_avec_nulls():
    ...
    result = df.select(expr_changement_avant_apres("avant_Value", "apres_Value").alias("changement"))
...
def test_expr_changement_avant_apres_avec_nombres():
    ...
    result = df.select(expr_changement_avant_apres("avant_Puissance", "apres_Puissance").alias("changement"))
...
def test_expr_changement_index_detecte_changements():
    ...
    result = df.select(expr_changement_index().alias("changement"))
...
def test_expr_impacte_energie_composition_complete():
    ...
    result = df.select(
        [
            expr_changement_avant_apres("avant_id_calendrier_distributeur", "apres_id_calendrier_distributeur").alias(
                "change_cal"
            ),
            expr_changement_index().alias("change_index"),
            expr_changement_avant_apres("avant_formule_tarifaire_acheminement", "formule_tarifaire_acheminement").alias(
                "change_fta"
            ),
...
def test_expr_evenement_entree():
    ...
    result = df.select(expr_evenement_entree().alias("entree"))
...
def test_expr_evenement_sortie():
    ...
    result = df.select(expr_evenement_sortie().alias("sortie"))
...
def test_expr_date_entree_periode():
    ...
    ).with_columns(pl.col("date_evenement").str.strptime(pl.Datetime, "%Y-%m-%d").dt.replace_time_zone("Europe/Paris"))

    result = (
        df.group_by("ref_situation_contractuelle")
        .agg(expr_date_entree_periode().alias("date_entree"))
...
def test_expr_date_sortie_periode_avec_sortie():
    ...
    result = df.group_by("ref_situation_contractuelle").agg(expr_date_sortie_periode().alias("date_sortie"))
...
def test_inserer_evenements_facturation_scenario_simple():
    ...
    resultat = (
        inserer_evenements_facturation(historique).collect().sort(["ref_situation_contractuelle", "date_evenement"])
    )
...
def test_inserer_evenements_facturation_plusieurs_pdl():
    ...
    facturation_pdl1 = resultat.filter(
        (pl.col("ref_situation_contractuelle") == "PDL1") & (pl.col("evenement_declencheur") == "FACTURATION")
    ).sort("date_evenement")

    facturation_pdl2 = resultat.filter(
        (pl.col("ref_situation_contractuelle") == "PDL2") & (pl.col("evenement_declencheur") == "FACTURATION")
    ).sort("date_evenement")
...
def test_inserer_evenements_facturation_integration():
    ...
    factu_fevrier = resultat.filter(pl.col("date_evenement").dt.strftime("%Y-%m-%d") == "2024-02-01").item(
        0, "puissance_souscrite_kva"
    )  # Hérité de MES (6.0)
    assert factu_fevrier == 6.0

    factu_mars = resultat.filter(pl.col("date_evenement").dt.strftime("%Y-%m-%d") == "2024-03-01").item(
        0, "puissance_souscrite_kva"
    )  # Hérité de MCT (9.0)
