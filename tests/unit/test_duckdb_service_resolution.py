...
l'import ignorerait le `DUCKDB__PATH` posé ensuite par le test — c'est
...
def test_get_freshness_resout_a_l_appel(tmp_path, monkeypatch):
    """DUCKDB__PATH posé après l'import est honoré ; l'erreur nomme le chemin résolu."""
    base_absente = tmp_path / "absente.duckdb"
    monkeypatch.setenv("DUCKDB__PATH", str(base_absente))
