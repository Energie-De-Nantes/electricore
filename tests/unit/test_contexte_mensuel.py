...
def stub_composer(monkeypatch):
    ...
    def _install(resultat):
        monkeypatch.setattr(
            ...
            lambda spine, chronologie, horizon: resultat,
        )
...
class TestChargerMoisParDefaut:
    ...
    def test_resout_le_dernier_mois_depuis_facturation(self, stub_composer):
        ...
        ctx = charger(spine=pl.LazyFrame({}), chronologie=pl.LazyFrame({}), mois=None)
...
class TestChargerFramesQuiPassent:
    ...
    def test_expose_les_quatre_frames_du_resultat(self, stub_composer):
        ...
        ctx = charger(spine=pl.LazyFrame({}), chronologie=pl.LazyFrame({}), mois="2024-01-01")
...
class TestChargerHorizon:
    ...
    def test_resout_l_horizon_par_defaut_quand_absent(self, monkeypatch):
        ...
        def _composer_capture(spine, chronologie, horizon):
        ...
        cm.charger(spine=pl.LazyFrame({}), chronologie=pl.LazyFrame({}), mois="2024-01-01")
    ...
    def test_propage_l_horizon_explicite(self, monkeypatch):
        ...
        cm.charger(spine=pl.LazyFrame({}), chronologie=pl.LazyFrame({}), mois="2024-01-01", horizon=horizon)
...
class TestContexteDuMois:
    ...
    def test_cable_les_loaders_et_delegue_a_charger(self, monkeypatch):
        """spine → branche abonnement, chronologie → mart énergie (sentinelles : l'inversion échoue)."""
        import electricore.core.builds.contexte_mensuel as cm

        spine_sentinel = pl.LazyFrame({"source": ["spine"]})
        ...
        assert captures["spine"] is spine_sentinel
