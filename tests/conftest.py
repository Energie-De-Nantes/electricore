...
def duckdb_with_sample_data(
    ...
    duckdb_test_connection.execute("CREATE TABLE flux_c15 AS SELECT * FROM minimal_historique_df")
    duckdb_test_connection.execute("CREATE TABLE flux_r151 AS SELECT * FROM minimal_releves_df")
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "wip: Tests en cours de développement (work in progress)")
