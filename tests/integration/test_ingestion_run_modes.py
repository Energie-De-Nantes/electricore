...
def _pipeline_courtcircuite(monkeypatch):
    ...
    def _fake_run(job):
        job.status = ingestion_service.StatutIngestion.completed

    monkeypatch.setattr(ingestion_service, "_run_pipeline", _fake_run)
    monkeypatch.setattr(ingestion_service, "is_running", lambda: False)
